...
public class QFourPhaseOne 
{
	...
	public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException 
	{
	    ...
	    String docName = ((FileSplit) context.getInputSplit()).getPath().getName(); // reading the documents sequentaillsy and getting the document number or the document name
	...
	Configuration conf = new Configuration();
        ...
        Job job = new Job(conf, "QFourPhaseOne");
    
	job.setOutputKeyClass(Text.class);
	job.setOutputValueClass(IntWritable.class);
	job.setJarByClass(QFourPhaseOne.class);
	...
	FileInputFormat.addInputPath(job, new Path(args[1])); //to accept three documents as inputs
