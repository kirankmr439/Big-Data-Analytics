...
public class QFourPhaseOneThree 
{
	...
	public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException 
	{
	    ...
	    String docName = ((FileSplit) context.getInputSplit()).getPath().getName(); //getting the document number or the document name
	    ...
	    {
		tempStr = (docPart[i]+" "+ docPart[i+1]) .replaceAll("\\p{P}", ""); //removing special character and punctuation from the word
	...
	Configuration conf = new Configuration();
        ...
        Job job = new Job(conf, "QFourPhaseOneThree");
    
	job.setOutputKeyClass(Text.class);
	job.setOutputValueClass(IntWritable.class);
	job.setJarByClass(QFourPhaseOneThree.class);
